...
struct capability_info vmx_procbased_ctls[21] =
{
...
struct capability_info vmx_procbased_ctls2[23] =
{
...
struct capability_info vmx_exit_ctls[11] =
{
...
struct capability_info vmx_entry_ctls[9] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Procbased_ctls Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vmx_procbased_ctls, 21, lo, hi);

	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("Procbased_ctls2 Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vmx_procbased_ctls2, 23, lo, hi);

	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vmx_exit_ctls, 11, lo, hi);

	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vmx_entry_ctls, 9, lo, hi);
}
